...
function HomeContent() {
    ...
    return (
      <div className="flex flex-wrap gap-4">
        {products.sort(
          ...
          <div
            key={product.id}
            className="relative border p-3 rounded-lg flex flex-col items-center justify-between w-64 min-h-60 group overflow-scroll"
          >
            <img
            ...
            />
            <h3 className="text-lg font-bold text-center break-words">{product.name}</h3>
            {product.stock === 0 ? (
  ...
  const addToCart = async (product: Product) => {
    if(session){
      ...
      setAddMessage("É necessário estar logado para adicionar itens ao carrinho")
    }
  }
